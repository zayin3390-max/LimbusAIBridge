...
- 小窗口下操作栏会换行，重译和报告可从“更多操作”进入；列表与编辑区改为上下排列，内容可以滚动。主要操作、设置保存和停止按钮保持可访问。
...
1. 关闭旧工具窗口，打开 1.2.1 或更新版本并扫描。
2. 切到“译名待核对”处理可疑内容，或切到“已有译文”重译原有 AI／手动译文。
3. 勾选需要重译的条目。可按角色名字或关键词搜索；“全选本页”只选当前页，“更多 → 选中全部筛选结果”包含其他页。
4. 点击“重译所选”。此按钮会重新调用配置的 API 并消耗额度；“翻译所选”仍跳过已有译文。
5. 审阅结果，退出游戏后“生成语言包”。

重译前先将所选条目的原始缓存及当前可见译文保存到 data/retranslation-history。新译文通过校验后才写入缓存；失败、停止或密钥错误时保留未完成条目的旧译文。重译失败的内容在“失败项”中显示，重新打开软件也可继续查看与重试。未勾选条目不会发送到 API；零协会已有人工译文不会被重译。
重译使用当前词表、角色规则和本地人工参考，不把历史 AI 译文作为本次请求的定名依据。对已经明确修正的硬币／金枝等错译，只需扫描后生成语言包，不必再次付费重译。
