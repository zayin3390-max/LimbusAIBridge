...
def role(kind,field):
    ...
    if kind.startswith('rpg-location:') and field=='text':return 'label'
...
def _make_index(scan):
    ...
    # Autogenerated model labels may omit JP while dialogue/enemy labels carry
    # both KR and JP. A partial signature may inherit ONE unambiguous superset;
    # two conflicting supersets must remain separate (no transitive guessing).
    scopes=collections.defaultdict(set)
    for label,source,scope in index['labels']:
        if scope[0]=='refs':scopes[source].add(scope)
    merged=collections.defaultdict(list)
    for (label,source,scope),slots in index['labels'].items():
        if scope[0]=='refs':
            original=set(scope[1])
            supersets=[other for other in scopes[source] if original < set(other[1])]
            maximal=[other for other in supersets
                     if not any(set(other[1]) < set(third[1]) for third in supersets)]
            if len(maximal)==1:scope=maximal[0]
        merged[(label,source,scope)].extend(slots)
    index['labels']=merged
    return index
...
def _reference_allowed(entry,source,term):
    ...
    # Multiword status names in mechanical descriptions are references even
    # without quotation marks. Ordinary narrative homonyms remain unbound.
    if term['role']=='status' and not narrative and len(WORDS.findall(source))>=2:return True
...
def _references(entry,terms,term_index,*,include_components=False):
    ...
    from .lexicon import term_locks
    concepts=[(m.start(),m.end(),len(name)) for name in term_locks(entry)
              for m in _matches(text,name)]
    for source in sorted(_relevant(text,term_index),key=lambda v:(-len(v),v)):
        ...
        matches=[m for m in _matches(text,source)
                 if not any(length>len(source) and a<=m.start() and m.end()<=b
                            for a,b,length in concepts)]
